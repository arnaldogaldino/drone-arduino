...
void MMA7455::calibrateOffset(float x_axis_offset, float y_axis_offset, float z_axis_offset)
{
  _x_axis_offset = x_axis_offset;
  _y_axis_offset = y_axis_offset;
  _z_axis_offset = z_axis_offset;
}
unsigned char MMA7455::readAxis(char axis)
{
  Wire.beginTransmission(MMA7455_ADDRESS);
  if(axis == 'x' || axis == 'X')
  {
...
AccelerometerScaled  MMA7455::ReadScaledAxis() {
